...
/** Expressions for the language without variability (cf. chapter 6 slides
  * 70/71).
  */
enum Expression {
...
/** Expressions for the language with variability (cf. chapter 6 slides 72-74).
  */
enum VExpression {
  case Const(c: Constant)
  case Id(id: Identifier)
    ...
    obj match {
      case
        ...
        dom()
          .union(typeB.dom())
          .forall(t => {
            val formulaA = formulaForType(t).getOrElse(Formulas.False)
            val formulaB = typeB.formulaForType(t).getOrElse(Formulas.False)
